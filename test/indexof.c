...
int main() {
    ...
    //     printf("indexOf: %c is %02d\n", acid[i], indexOf(acid[i]));
    // }
    for (int i = 0, j = 0; i < I5N_SIXBITS_SIZE; i += 3, j += 4) {
        ...
        dec[j]   = sixbitstable[enc[i] >> 2];
        dec[j+1] = sixbitstable[(enc[i] & 0x3) << 4 | (enc[i+1] & 0xf0) >> 4];
        dec[j+2] = sixbitstable[(enc[i+1] & 0x0f) << 2 | (enc[i+2] & 0xC0) >> 6];
        dec[j+3] = sixbitstable[(enc[i+2] & 0x3f)];
    }
    printf("decoded: %.*s\n", (int)I5N_ASCII_SIZE, dec);
    ...
    printf("encoded: [0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x]\n", enc[0], enc[1], enc[2], enc[3], enc[4], enc[5]);
    return 0;
}
